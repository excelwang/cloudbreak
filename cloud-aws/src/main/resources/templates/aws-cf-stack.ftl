  ...
  <#if enableInstanceProfile && !existingRole>
    ...
    "VPC" : {
      ...
      "Properties" : {
        ...
        "Tags" : [
          ...
          { "Key" : "cb_resource_type", "Value" : "${network_resource}" }
        ]
      }
    },
    </#if>
	...
	"AmbariNodes${group.groupName?replace('_', '')}" : {
      ...
      "Properties" : {
        ...
        "Tags" : [ { "Key" : "Name", "Value" : { "Fn::Join" : ["-", [ { "Ref" : "StackName" }, "${group.groupName}"]] }, "PropagateAtLaunch" : "true" },
        		   ...
        		   { "Key" : "cb_resource_type", "Value" : "${securitygroup_resource}", "PropagateAtLaunch" : "true" },
    ...
    "ClusterNodeSecurityGroup${group.groupName?replace('_', '')}" : {
      ...
      "Properties" : {
        ...
        "Tags" : [{ "Key" : "cb_resource_type", "Value" : "${securitygroup_resource}"}],
        <#if existingVPC>
        "VpcId" : { "Ref" : "VPCId" },
        <#else>
        "VpcId" : { "Ref" : "VPC" },
